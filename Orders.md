...
For a list of all associated endpoints, visit the [Order Taxes](https://developer.bigcommerce.com/api-reference/store-management/orders/order-taxes) reference page.

Tax will be calculated based on the tax setting rules specified in the store, except in the case of automatic taxes. However, in both cases, you can optionally override the tax values by specifying `price_inc_tax` and `price_ex_tax`. 
...
Once an Order has products, a billing address and at least one shipping address, an order shipment can be created. Order shipments are a way to mark an order as shipped with the shipping information.
